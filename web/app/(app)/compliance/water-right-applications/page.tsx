...
export default function WaterRightApplicationsPage() {
  const { data } = useErpReference()
  const applications = data.WaterRightApplications ?? []
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Water Right applications submitted through the public portal
          </p>
        </div>
      </div>
                  ...
                  <TableRow key={app.id}>
                    ...
                    <TableCell>
                      <Badge className={cn('font-normal', statusBadgeClass(app.status))}>
                        {waterRightApplicationStatusLabels[app.status] ?? app.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/compliance/water-right-applications/${app.id}`}>Review</Link>
                      </Button>
                    </TableCell>
                  </TableRow>
                ))
