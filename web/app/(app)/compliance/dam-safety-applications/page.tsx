...
export default function DamSafetyApplicationsPage() {
  const { data } = useErpReference()
  const applications = data.damSafetyApplications ?? []
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Dam Safety applications submitted through the public portal
          </p>
        </div>
      </div>
                  ...
                  <TableRow key={app.id}>
                    ...
                    <TableCell>
                      <Badge className={cn('font-normal', statusBadgeClass(app.status))}>
                        {damSafetyApplicationStatusLabels[app.status] ?? app.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/compliance/dam-safety-applications/${app.id}`}>Review</Link>
                      </Button>
                    </TableCell>
                  </TableRow>
                ))
