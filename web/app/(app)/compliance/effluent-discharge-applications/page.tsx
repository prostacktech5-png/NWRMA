...
export default function EffluentDischargeApplicationsPage() {
  const { data } = useErpReference()
  const applications = data.effluentDischargeApplications ?? []
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground sm:text-3xl">
            Effluent discharge applications
          </h1>
          <p className="text-muted-foreground">
            Effluent discharge applications submitted through the public portal
          </p>
        </div>
      </div>
                  ...
                  <TableRow key={app.id}>
                    ...
                    <TableCell>
                      <Badge className={cn('font-normal', statusBadgeClass(app.status))}>
                        {effluentDischargeApplicationStatusLabels[app.status] ?? app.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/compliance/effluent-discharge-applications/${app.id}`}>Review</Link>
                      </Button>
                    </TableCell>
                  </TableRow>
                ))
