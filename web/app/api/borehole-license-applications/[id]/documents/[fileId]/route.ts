...
export async function GET(
  ...
  return tryRespondWithDbSetupHint(async () => {
    ...
    if (!canReviewLicenseApplications(viewer) && !canAccessBankReceiptValidationDesk(viewer)) {
    ...
    const application = payload.licenseApplications.find((a) => a.id === id)
    if (!application) {
      return Response.json({ error: 'Application not found.' }, { status: 404 })
    }

    const doc = findApplicationDocument(application, fileId)
    if (!doc?.storageKey) {
      return Response.json(
    ...
    try {
      const { buffer, mimeType } = await readLicenseFile(doc.storageKey)
