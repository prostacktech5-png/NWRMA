...
import { canReviewEffluentDischargeApplications } from '@/lib/effluent-discharge-application'
import { canAccessBankReceiptValidationDesk } from '@/lib/bank-receipt-validation-desk'
import {
  findEffluentDischargeDocument,
  readEffluentDischargeFile,
...
export async function GET(
  ...
  return tryRespondWithDbSetupHint(async () => {
    ...
    if (
      !canReviewEffluentDischargeApplications(viewer) &&
      !canAccessBankReceiptValidationDesk(viewer)
    ) {
    ...
    const application = (payload.effluentDischargeApplications ?? []).find((a) => a.id === id)
    if (!application) {
      return Response.json({ error: 'Application not found.' }, { status: 404 })
    }

    const doc = findEffluentDischargeDocument(application, fileId)
    if (!doc?.storageKey) {
      return Response.json(
    ...
    try {
      const { buffer, mimeType } = await readEffluentDischargeFile(doc.storageKey)
