...
import {
  canReviewEffluentDischargeApplications,
  isReviewableEffluentDischargeStatus,
} from '@/lib/effluent-discharge-application'
import { additionalInfoRequestPayload } from '@/lib/application-amendment'
import { notifyEffluentDischargeApplicantForStatus } from '@/lib/effluent-discharge-notify'
import { resolveDemoViewerFromRequest } from '@/lib/demo-viewer-server'
import type { EffluentDischargeApplicationStatus } from '@/lib/types'
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  return tryRespondWithDbSetupHint(async () => {
    ...
    if (!canReviewEffluentDischargeApplications(viewer)) {
      return Response.json(
        {
          error:
            'Only Hydrological Application processing unit staff, Compliance staff, or administrators can review effluent discharge applications.',
        },
        { status: 403 }
      )
    }
    ...
    if (status && !isReviewableEffluentDischargeStatus(status)) {
      return Response.json(
        {
          error:
    ...
    const applications = payload.effluentDischargeApplications ?? []
    ...
    if (status === 'additional_info_required' && reviewNote) {
      const issued = additionalInfoRequestPayload('effluent-discharge', id, existing, reviewNote)
    ...
    let updated = {
      ...existing,
      ...additionalPatch,
      status: status ? (status as EffluentDischargeApplicationStatus) : existing.status,
      reviewNote:
    ...
    const effluentDischargeApplications = [...applications]
    ...
    if (
      ...
      try {
        await notifyEffluentDischargeApplicantForStatus(updated, status, { amendUrl })
