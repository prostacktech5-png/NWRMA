...
import { canReviewWaterRightApplications } from '@/lib/water-right-application'
import { canAccessBankReceiptValidationDesk } from '@/lib/bank-receipt-validation-desk'
import { findWaterRightDocument, readWaterRightFile } from '@/lib/water-right-file-store'
...
export async function GET(
  ...
  return tryRespondWithDbSetupHint(async () => {
    ...
    if (!canReviewWaterRightApplications(viewer) && !canAccessBankReceiptValidationDesk(viewer)) {
    ...
    const application = (payload.waterRightApplications ?? []).find((a) => a.id === id)
    if (!application) {
      return Response.json({ error: 'Application not found.' }, { status: 404 })
    }

    const doc = findWaterRightDocument(application, fileId)
    if (!doc?.storageKey) {
      return Response.json(
    ...
    try {
      const { buffer, mimeType } = await readWaterRightFile(doc.storageKey)
