import {
  createWaterRightApplicationFromForm,
  newWaterRightApplicationId,
} from '@/lib/water-right-application'
import {
  ALL_WATER_RIGHT_SLOT_IDS,
  WATER_RIGHT_REQUIRED_SLOTS,
...
import {
  ...
  newLicenseFileId,
  saveWaterRightFileFromBuffer,
} from '@/lib/water-right-file-store'
import { notifyWaterRightApplicationReceived } from '@/lib/water-right-notify'
...
function isWaterRightSlot(value: string): value is WaterRightDocumentSlotId {
  return (ALL_WATER_RIGHT_SLOT_IDS as string[]).includes(value)
}
...
export async function POST(req: Request) {
  ...
  const applicationId = newWaterRightApplicationId()
  ...
  return tryRespondWithDbSetupHint(async () => {
    ...
    const documents = {} as Record<WaterRightDocumentSlotId, WaterRightDocumentMeta[]>
    for (const id of ALL_WATER_RIGHT_SLOT_IDS) documents[id] = []
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (!isWaterRightSlot(slotId)) continue
      if (WATER_RIGHT_REQUIRED_SLOTS.some((s) => s.id === slotId)) continue
    ...
    const app = createWaterRightApplicationFromForm({
      form: parsed.data,
      documents,
      existing: payload.waterRightApplications ?? [],
      applicationId,
    })

    payload.waterRightApplications = [...(payload.waterRightApplications ?? []), app]
    payload = linkIntakeToApplication(payload, intake.id, app.id)
    await saveErpReferencePayload(payload)

    let emailWarning: string | undefined
    try {
      await notifyWaterRightApplicationReceived(app)
    } catch (e) {
      emailWarning =
