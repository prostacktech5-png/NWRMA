import {
  createDamSafetyApplicationFromForm,
  newDamSafetyApplicationId,
} from '@/lib/dam-safety-application'
import {
  ALL_DAM_SAFETY_SLOT_IDS,
  DAM_SAFETY_REQUIRED_SLOTS,
...
import {
  ...
  newLicenseFileId,
  saveDamSafetyFileFromBuffer,
} from '@/lib/dam-safety-file-store'
import { notifyDamSafetyApplicationReceived } from '@/lib/dam-safety-notify'
...
function isDamSlot(value: string): value is DamSafetyDocumentSlotId {
  return (ALL_DAM_SAFETY_SLOT_IDS as string[]).includes(value)
}
...
export async function POST(req: Request) {
  ...
  const applicationId = newDamSafetyApplicationId()
  ...
  return tryRespondWithDbSetupHint(async () => {
    ...
    const documents = {} as Record<DamSafetyDocumentSlotId, DamSafetyDocumentMeta[]>
    for (const id of ALL_DAM_SAFETY_SLOT_IDS) documents[id] = []
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (!isDamSlot(slotId)) continue
      if (DAM_SAFETY_REQUIRED_SLOTS.some((s) => s.id === slotId)) continue
    ...
    const app = createDamSafetyApplicationFromForm({
      form: parsed.data,
      documents,
      existing: payload.damSafetyApplications ?? [],
      applicationId,
    })

    payload.damSafetyApplications = [...(payload.damSafetyApplications ?? []), app]
    payload = linkIntakeToApplication(payload, intake.id, app.id)
    await saveErpReferencePayload(payload)

    let emailWarning: string | undefined
    try {
      await notifyDamSafetyApplicationReceived(app)
    } catch (e) {
      emailWarning =
