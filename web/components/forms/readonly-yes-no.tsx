...
export function ReadOnlyYesNoDisplay({
  label,
  included,
  reasonIfNo,
}: {
  ...
  return (
    <div
      className={cn(
        ...
        warn && 'rounded-md bg-amber-50/80 px-2 py-1 ring-1 ring-amber-200'
      )}
    >
      <p className="text-xs font-medium text-muted-foreground">{label}</p>
      <p className="text-sm text-foreground">{isBlank(included) ? '—' : text}</p>
