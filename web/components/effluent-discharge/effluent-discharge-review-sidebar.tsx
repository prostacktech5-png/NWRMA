...
import { formatEffluentDischargeApplicantEmailList } from '@/lib/effluent-discharge-application'
...
import type { EffluentDischargeApplication, EffluentDischargeApplicationStatus } from '@/lib/types'
import { cn } from '@/lib/utils'

function statusBadgeClass(status: EffluentDischargeApplicationStatus): string {
  switch (status) {
    case 'approved':
...
export function EffluentDischargeReviewSidebar({
  application,
  onUpdated,
}: {
  application: EffluentDischargeApplication
  onUpdated: (app: EffluentDischargeApplication) => void
}) {
  ...
  const patchStatus = async (
    ...
    try {
      const res = await fetch(`/api/effluent-discharge-applications/${application.id}`, {
        method: 'PATCH',
        headers: {
      ...
      if (status !== 'under_review') {
        if (data.emailWarning) {
          ...
          parts.push(
            `Notification email sent to ${formatEffluentDischargeApplicantEmailList(data.application ?? application)}.`
          )
        }
      }
  ...
  return (
    <Card className="sticky top-24 border-0 shadow-lg">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">Effluent discharge — portal submission</p>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded-lg border border-gray-200 bg-white p-3 text-sm">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <Badge className={cn('font-normal', statusBadgeClass(application.status))}>
              {effluentDischargeApplicationStatusLabels[application.status] ?? application.status}
            </Badge>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-3 text-sm">
          ...
          <p className="mt-1 text-gray-600">{formatEffluentDischargeApplicantEmailList(application)}</p>
        </div>

        <div className="rounded-lg bg-gray-50 p-4">
          <h4 className="font-semibold text-[#0a2647]">Required attachments</h4>
          <ul className="mt-2 space-y-3 text-sm text-gray-600">
            {EFFLUENT_DISCHARGE_REQUIRED_SLOTS.map((doc) => {
        ...
        <div className="space-y-2">
          <Label htmlFor="effluentReviewNote">Review notes (included in applicant emails)</Label>
          <Textarea
            id="effluentReviewNote"
            rows={3}
        ...
        <RequestAdditionalInfoDialog
          ...
          applicantLabel={formatEffluentDischargeApplicantEmailList(application)}
