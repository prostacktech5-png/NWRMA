...
function statusBadgeClass(status: DamSafetyApplicationStatus): string {
  switch (status) {
    case 'approved':
...
export function DamSafetyReviewSidebar({
  application,
  onUpdated,
}: {
  application: DamSafetyApplication
  onUpdated: (app: DamSafetyApplication) => void
}) {
    ...
    try {
      ...
      if (status !== 'under_review') {
        if (data.emailWarning) {
          ...
          parts.push(
            `Notification email sent to ${formatDamSafetyApplicantEmailList(data.application ?? application)}.`
          )
        }
      }
  ...
  return (
    <Card className="sticky top-24 border-0 shadow-lg">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">Dam safety application — portal submission</p>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded-lg border border-gray-200 bg-white p-3 text-sm">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <Badge className={cn('font-normal', statusBadgeClass(application.status))}>
              {damSafetyApplicationStatusLabels[application.status] ?? application.status}
            </Badge>
        ...
        <div className="space-y-2">
          <Label htmlFor="damReviewNote">Review notes (included in applicant emails)</Label>
          <Textarea
            id="damReviewNote"
            rows={3}
