...
import { emptyWaterRightDocumentFiles } from '@/lib/water-right-documents'
import { WATER_RIGHT_REQUIRED_SLOTS } from '@/lib/water-right-documents'
import {
  ACTIVITY_SECTION_KEYS,
  ACTIVITY_SECTION_LABELS,
  createDefaultWaterRightForm,
  WATER_USE_PURPOSES,
  waterRightFormSchema,
  type WaterRightFormPayload,
...
import {
  WaterRightDocumentUploadGrid,
  type WaterRightFilesState,
} from '@/components/nwrma-site/online-forms/water-right-document-upload-grid'
import {
  FormField as Field,
  FormFieldLabel,
  FormRequiredLegend,
...
export function WaterRightForm({ title, pdfPath }: { title: string; pdfPath?: string }) {
  const [step, setStep] = useState(0)
  const [form, setForm] = useState<WaterRightFormPayload>(createDefaultWaterRightForm)
  const [documents, setDocuments] = useState<WaterRightFilesState>(emptyWaterRightDocumentFiles())
  ...
  const validateStep = (index: number): string | null => {
    ...
    if (index === 1) {
      ...
      const emailCheck = waterRightFormSchema.shape.email.safeParse(form.email.trim())
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await postPublicApplication('/api/public/water-right-applications', body)
      ...
      <>
        <div className="nwrma-page-body">
          <div className="nwrma-container px-4 nwrma-form-success">
            ...
            <p>
              Your Water Right application has been received. Reference:{' '}
          ...
          <FormPaymentGateWizardStep gate={paymentGate} step={step} targetStep={10}>
            <FormSection title="Review & submit">
              <ul className="nwrma-review-summary">
                ...
                <li><strong>Documents:</strong> {WATER_RIGHT_REQUIRED_SLOTS.filter((s) => documents[s.id]?.length).length} of {WATER_RIGHT_REQUIRED_SLOTS.length} required</li>
              </ul>
            </FormSection>
          </FormPaymentGateWizardStep>
