...
import {
  DamSafetyDocumentUploadGrid,
  type DamSafetyFilesState,
} from '@/components/nwrma-site/online-forms/dam-safety-document-upload-grid'
import {
  FormField as Field,
  FormFieldLabel,
  FormRequiredLegend,
...
export function DamSafetyForm({ title, pdfPath }: { title: string; pdfPath?: string }) {
  const [step, setStep] = useState(0)
  const [form, setForm] = useState<DamSafetyFormPayload>(createDefaultDamSafetyForm)
  const [documents, setDocuments] = useState<DamSafetyFilesState>(emptyDamSafetyDocumentFiles())
  ...
  const validateStep = (index: number): string | null => {
    ...
    if (index === 1) {
      ...
      const emailCheck = damSafetyFormSchema.shape.email.safeParse(form.email.trim())
      if (!emailCheck.success) return 'Enter a valid email address.'
      if (!form.ownershipType.trim()) return 'Ownership type is required.'
    }
    if (index === 2) {
      ...
      if (form.purposes.length === 0) {
        return 'Select at least one purpose/type of water use.'
      }
    }
    if (index === 4) {
      if (!form.damLocationTown.trim()) return 'Dam location — Town is required.'
      if (!form.damLocationDistrict.trim()) return 'Dam location — District is required.'
      if (!form.damLocationRegion.trim()) return 'Dam location — Region is required.'
      if (!form.damGpsCoordinates.trim()) return 'Dam GPS coordinates are required.'
      if (!form.damStatus.trim()) return 'Dam status is required.'
      if (!form.waterSourceType.trim()) return 'Type of water source is required.'
      if (!form.damClass.trim()) return 'Dam class is required.'
      if (!form.damMaterial.trim()) return 'Dam material is required.'
      if (!form.damFunction.trim()) return 'Dam function is required.'
      if (!form.damClassAppliedFor.trim()) return 'Dam class applied for is required.'
    }
    if (index === 5 && !form.projectDescription.trim()) {
    ...
    if (index === 9) {
      if (!form.declarationSignature.trim() || !form.declarationPrintName.trim()) {
        return 'Declaration signature and print name are required.'
      }
    }
    return null
  }
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await postPublicApplication('/api/public/dam-safety-applications', body)
      const data = (await res.json()) as ApiValidationBody & { reference?: string }
      if (!res.ok) {
        setError(apiValidationErrorMessage(data))
        const firstField = Object.keys(data.details?.fieldErrors ?? {})[0]
        if (firstField) {
          setStep(permitApplicationPathToStep([firstField]))
        }
        return
      }
      ...
      <>
        <div className="nwrma-page-body">
          <div className="nwrma-container px-4 nwrma-form-success">
            ...
            <p>
              Your Dam Safety application has been received. Reference:{' '}
            ...
            <FormSection title="1.0 Applicant details">
              <div className="nwrma-field-grid">
                ...
                <Field label="Licence type">
                  <select className="nwrma-field-input" value={form.licenceType} onChange={(e) => patch({ licenceType: e.target.value as 'new' | 'renewal' })}>
                    <option value="new">New</option>
                    <option value="renewal">Renewal</option>
                  </select>
                </Field>
                {form.licenceType === 'renewal' ? (
                  <Field required={false} label="Licence ID number (renewal)">
                    <input className="nwrma-field-input" value={form.renewalLicenceId} onChange={(e) => patch({ renewalLicenceId: e.target.value })} />
                  </Field>
                ) : null}
              </div>
          ...
          <FormPaymentGateWizardStep gate={paymentGate} amend={amend} step={step} targetStep={2}>
            <FormSection title="2.0–3.0 Location & purpose of water use">
              ...
              <FormFieldLabel required as="p" className="mt-4">
                Purpose/type of water use (select all that apply)
              </FormFieldLabel>
          ...
          <FormPaymentGateWizardStep gate={paymentGate} amend={amend} step={step} targetStep={4}>
            <FormSection title="5.0 Data on the (proposed) dam use">
              <div className="nwrma-field-grid">
                ...
                <Field label="District"><input className="nwrma-field-input" value={form.damLocationDistrict} onChange={(e) => patch({ damLocationDistrict: e.target.value })} /></Field>
                <Field label="Region"><input className="nwrma-field-input" value={form.damLocationRegion} onChange={(e) => patch({ damLocationRegion: e.target.value })} /></Field>
                <Field label="GPS coordinates" className="nwrma-field--full"><input className="nwrma-field-input" value={form.damGpsCoordinates} onChange={(e) => patch({ damGpsCoordinates: e.target.value })} /></Field>
                <Field label="Current status of dam"><input className="nwrma-field-input" value={form.damStatus} onChange={(e) => patch({ damStatus: e.target.value })} placeholder="Construction / Operational" /></Field>
                ...
                <Field required={false} label="Storage types (tailings, hydrocarbon, etc.)" className="nwrma-field--full"><textarea className="nwrma-field-input" rows={2} value={form.storageTypes} onChange={(e) => patch({ storageTypes: e.target.value })} /></Field>
                <Field required={false} label="Communities upstream" className="nwrma-field--full"><textarea className="nwrma-field-input" rows={2} value={form.communitiesUpstream} onChange={(e) => patch({ communitiesUpstream: e.target.value })} /></Field>
                <Field required={false} label="Communities downstream" className="nwrma-field--full"><textarea className="nwrma-field-input" rows={2} value={form.communitiesDownstream} onChange={(e) => patch({ communitiesDownstream: e.target.value })} /></Field>
                <Field required={false} label="Water users upstream" className="nwrma-field--full"><textarea className="nwrma-field-input" rows={2} value={form.waterUsersUpstream} onChange={(e) => patch({ waterUsersUpstream: e.target.value })} /></Field>
                <Field required={false} label="Water users downstream" className="nwrma-field--full"><textarea className="nwrma-field-input" rows={2} value={form.waterUsersDownstream} onChange={(e) => patch({ waterUsersDownstream: e.target.value })} /></Field>
                <Field label="Class of dam"><input className="nwrma-field-input" value={form.damClass} onChange={(e) => patch({ damClass: e.target.value })} placeholder="Small / Medium / Large" /></Field>
                <Field label="Material of dam"><input className="nwrma-field-input" value={form.damMaterial} onChange={(e) => patch({ damMaterial: e.target.value })} /></Field>
                <Field label="Function of dam"><input className="nwrma-field-input" value={form.damFunction} onChange={(e) => patch({ damFunction: e.target.value })} /></Field>
                <Field required={false} label="Types of dams"><input className="nwrma-field-input" value={form.damTypes} onChange={(e) => patch({ damTypes: e.target.value })} /></Field>
                <Field label="Class of dam being applied for"><input className="nwrma-field-input" value={form.damClassAppliedFor} onChange={(e) => patch({ damClassAppliedFor: e.target.value })} /></Field>
                <Field required={false} label="Actual capacity (if in progress)"><input className="nwrma-field-input" value={form.actualCapacity} onChange={(e) => patch({ actualCapacity: e.target.value })} /></Field>
                <Field required={false} label="Rate of use — Daily"><input className="nwrma-field-input" value={form.rateOfUseDaily} onChange={(e) => patch({ rateOfUseDaily: e.target.value })} /></Field>
                ...
                <Field required={false} label="Method of determining consumption" className="nwrma-field--full"><textarea className="nwrma-field-input" rows={2} value={form.consumptionDeterminationMethod} onChange={(e) => patch({ consumptionDeterminationMethod: e.target.value })} /></Field>
              </div>
            </FormSection>
          </FormPaymentGateWizardStep>
          ...
          <FormPaymentGateWizardStep gate={paymentGate} amend={amend} step={step} targetStep={10}>
            <FormSection title="Review & submit">
              <ul className="nwrma-review-summary">
                ...
                <li><strong>Documents:</strong> {DAM_SAFETY_REQUIRED_SLOTS.filter((s) => documents[s.id]?.length).length} of {DAM_SAFETY_REQUIRED_SLOTS.length} required</li>
              </ul>
            </FormSection>
          </FormPaymentGateWizardStep>
