...
import { homeNews, whatWeDo } from '../data/news'
import './Home.css'
...
export function Home() {
  return (
    <>
      <Helmet>
        <title>Home - National Water Resources Management Agency</title>
      </Helmet>
            ...
            {whatWeDo.map((item) => (
              <div key={item.title} className="what-we-do__item">
                <div className="what-we-do__icon">
                  {item.image ? (
                    <ImageWithFallback localSrc={item.image} alt={item.title} width={225} height={225} />
                  ) : (
                    <div className="what-we-do__placeholder" />
                  )}
                </div>
      ...
      <section className="section section--resources">
        ...
        <div className="container section--resources__content">
          ...
          <div className="grid grid--3 resources-grid">
            <Link to="/data" className="resource-link">
              <i className="fas fa-sliders-h resource-icon" />
              <h5>
                <strong>Data</strong>
              </h5>
            </Link>
            <Link to="/maps" className="resource-link">
              <i className="fas fa-map-marker-alt resource-icon" />
              <h5>
                <strong>Maps</strong>
              </h5>
            </Link>
            <Link to="/publications" className="resource-link">
      ...
      <section className="section section--news">
        ...
        <div className="container">
          ...
          <p className="text-center" style={{ marginTop: '2rem' }}>
            <Link to="/news" className="btn btn--gradient">
              More News
            </Link>
          </p>
        </div>
